...
## Step 9 `[AGENT]`：Lighthouseでパフォーマンスを計測する

### やること

Lighthouse CLIを使って、本番URL（またはローカル）のパフォーマンスを計測し、スコアを報告する。

### 前提

- ローカル計測の場合：`npm run dev` が起動していること
- 本番計測の場合：Vercelにデプロイ済みであること
...
```json
"lighthouse": "lighthouse http://localhost:3000 --output=json --output-path=./lighthouse-report.json --chrome-flags=\"--headless\" --only-categories=performance,accessibility,best-practices,seo --form-factor=mobile --quiet"
```

**インストールするパッケージ**

- `lighthouse` — Google公式のLighthouse CLI（devDependencies）

### 手順

1. `lighthouse` パッケージをインストールする（`npm install -D lighthouse`）
2. `package.json` の `scripts` に上記コマンドを追加する
3. `npm run lighthouse` を実行して計測する
4. `lighthouse-report.json` からスコアを読み取ってユーザーに報告する
5. `.gitignore` に `lighthouse-report.json` を追加する
...
> 💡 スコアが低い項目があっても、このPhaseでは「計測して現状を把握する」ことが目的。改善は必要に応じて後から行う。

> ⚠️ 開発モード（`npm run dev`）で計測するとスコアが低くなる。正確なスコアを計測するには `npm run build && npm start` でプロダクションビルドを起動してから計測すること。

### エージェントへの指示例

> `lighthouse` パッケージをdevDependenciesにインストールし、`package.json` に `lighthouse` スクリプトを追加してください。実行して計測結果のスコアを報告してください。`lighthouse-report.json` は `.gitignore` に追加してください。
