...
- **Database Password：** 安全なパスワードを設定し、必ずメモしておくこと(Rcb7e3O5eLsimahp)
...
Supabaseのダッシュボード「Project Settings → API」から以下を取得する。

| 項目             | 用途                                          |
| ---------------- | --------------------------------------------- |
| Project URL      | SupabaseへのAPIエンドポイント                 |
| anon public key  | 公開して良いAPIキー（フロントエンドから使用） |
| service_role key | 秘密のAPIキー（サーバーサイドのみで使用）     |

「Project Settings → Database」から以下を取得する。

| 項目                    | 用途                          |
| ----------------------- | ----------------------------- |
| Connection string (URI) | PrismaがDBに接続するためのURL |

> ⚠️ `service_role key` と `DATABASE_URL` は絶対にGitHubに公開しないこと。
...
`.env.local` の値をVercelのダッシュボードにも設定する必要がある。「Project Settings → Environment Variables」から全変数を追加する。

`.env.local` はローカル開発用、Vercelの環境変数は本番用。この2つを常に同期しておくこと。
