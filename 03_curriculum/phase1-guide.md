...
- **Region（リージョン）：** Northeast Asia (Tokyo) を選択
  - ユーザーに近いリージョンを選ぶとレスポンスが速くなるため
- **Database Password：** 安全なパスワードを設定し、必ずメモしておくこと

- **Enable Data API：** ON（チェックを入れる）

  SupabaseのクライアントライブラリやAPIを通じてDBにアクセスするための設定。このアプリではSupabase Authを使うため必須。

- **Enable automatic RLS：** ON（チェックを入れる）

  RLSとはRow Level Security（行レベルセキュリティ）の略で、「誰がどの行にアクセスできるか」をDB側で制御する仕組み。
  このアプリではPrismaのマイグレーションでテーブルを作成するが、SQLやマイグレーション経由で作ったテーブルはRLSがデフォルトでOFFになる。RLSがOFFのテーブルはSupabaseのAPIを通じて誰でも全データにアクセスできる状態になるため非常に危険。
  automatic RLSをONにすると、マイグレーションで作られたテーブルにも自動でRLSが有効になるので、この設定を必ずONにすること。
...
**Project URL**
「Integrations → Data API」を開き、「API URL」をコピーする。

**Publishable key / Secret key**
「Project Settings → API Keys」を開き、以下を取得する。
...
画面上部の「**Connect**」ボタンをクリック → 「Connection String」タブ → Type: URI、Method: Direct connection を選択 → 表示された文字列をコピーする。
...
`create-next-app` で作成したプロジェクトには最初から `.gitignore` があり、`.env.local` は除外対象に含まれている。念のため確認すること。
...
Phase 1はアプリの初期設定なので、例外として `main` に直接pushしてよい。Phase 2以降は必ずfeatureブランチを使うこと。
...
「Environment Variables」セクションを展開し、`.env.local` の値を設定する。

> 💡 `.env.local` はドット始まりの隠しファイルのためMacのFinderでは表示されない。`Command + Shift + .` で表示/非表示を切り替えられる。

「**Import .env**」ボタンを使うと `.env.local` をそのままアップロードして一括設定できる。アップロード後に以下の4つが正しく入っていることを確認すること。
