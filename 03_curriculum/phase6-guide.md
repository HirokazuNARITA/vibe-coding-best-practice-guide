...
> **① イベントを選択する**
>
> 1. Stripeダッシュボード →「開発者」→「ワークベンチ」→「Webhook」を開く
> 2. 「＋ 送信先を追加する」をクリック
> 3. 「イベントのリッスン元」は **「お客様のアカウント」** を選択
> 4. 「イベント」の検索欄に `checkout.session.completed` と入力して選択する
> 5. 「続行 →」をクリック
>
> **② 送信先のタイプを選択する**
>
> 1. **「Webhook エンドポイント」** を選択する
> 2. 「続行 →」をクリック
>
> **③ 宛先を設定する**
>
> 1. 「エンドポイント URL」にVercelのURLを入力する
>    ```
>    https://YOUR_APP_URL/api/stripe/webhook
>    ```
>    （YOUR_APP_URLは実際のVercel URLに置き換える）
> 2. 「送信先を作成する」をクリック
> 3. 作成後に表示される詳細ページで「署名シークレット」をコピーする
>
> 完了したら教えてください。
